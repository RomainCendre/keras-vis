...
def visualize_cam_with_losses(input_tensor, losses, seed_input, penultimate_layer, grad_modifier=None):
    ...
    output_dims = np.squeeze(penultimate_output_value,axis=0).shape[0:2] #utils.get_img_shape(penultimate_output)[2:]
    ...
    if seed_input.ndim == 4:
        input_dims = seed_input.shape[1:3]# utils.get_img_shape(input_tensor)[2:]
    else:
        input_dims = seed_input.shape[0:2]
